...
const USERS_API     = "https://localhost:7293/User/GetAllUsers";
const BRANCHES_API  = "https://localhost:7293/Branch/GetAllBranch";
...
document.getElementById("addItemBtn").addEventListener("click", () => {
    ...
    if (!medId)                { alert("Pick a medicine."); return; }
    ...
    newItems.push({ medicineId: medId, medicineName: med ? med.medicineName : `#${medId}`, quantity: qty, unitPrice: price });
    renderNewItems();
});
...
function renderNewItems() {
    ...
    let total = 0;
    body.innerHTML = newItems.map((it, i) => {
...
function removeNewItem(index) { newItems.splice(index, 1); renderNewItems(); }
...
document.getElementById("createOrderBtn").addEventListener("click", () => {
    ...
    if (newItems.length === 0) { alert("Add at least one order item."); return; }
    const payload = {
        userId, branchId,
        orderItems: newItems.map(it => ({ medicineId: it.medicineId, quantity: it.quantity, unitPrice: it.unitPrice }))
    };
    ...
    .then(async res => {
        if (res.ok) {
            ...
            loadOrders(); loadSalesSummary();
        } else { alert(`Could not place order: ${await res.text()}`); }
    })
...
function editOrder(id, userId, orderDate) {
    ...
    document.getElementById("editOrderDate").value = orderDate ? orderDate.slice(0, 16) : "";
...
document.getElementById("saveEditBtn").addEventListener("click", () => {
    ...
    .then(async res => {
        if (res.ok) { bootstrap.Modal.getInstance(document.getElementById("editOrderModal")).hide(); loadOrders(); }
...
function updateStatus(id, status) {
    fetch(`${API}/UpdateOrderStatus?id=${id}&status=${encodeURIComponent(status)}`, { method: "PATCH" })
    .then(async res => {
        if (!res.ok) { alert(`Status change failed: ${await res.text()}`); loadOrders(); }
        else { loadSalesSummary(); }
    })
...
function deleteOrder(id) {
    ...
    fetch(`${API}/DeleteOrder?id=${id}`, { method: "DELETE" })
        .then(async res => {
            if (res.ok || res.status === 204) { loadOrders(); loadSalesSummary(); }
...
document.getElementById("applyFilterBtn").addEventListener("click", () => {
    ...
    fetch(`${API}/FilterOrders?${params.toString()}`)
        .then(res => res.json()).then(renderOrders)
...
function loadDropdowns() {
    fetch(USERS_API).then(r => r.json()).then(users => {
        document.getElementById("orderUser").innerHTML =
            `<option value="">— select —</option>` +
            users.map(u => `<option value="${u.userId}">${u.username || u.email}</option>`).join("");
    }).catch(() => setDropdownError("orderUser"));

    fetch(BRANCHES_API).then(r => r.json()).then(branches => {
        document.getElementById("orderBranch").innerHTML =
            `<option value="">— select —</option>` +
            branches.map(b => `<option value="${b.branchId}">${b.branchName || ("Branch " + b.branchId)}</option>`).join("");
...
function setDropdownError(id) {
    document.getElementById(id).innerHTML = `<option value="">(couldn't load)</option>`;
}
